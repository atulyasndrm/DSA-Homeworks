...
node* createMatrixrow(int mno){
    int rw,cl,data,mn,no; node *a,*b,*rtemp,*row,*temp;int flag=0; 
    if(mno==1){
     scanf("%d",&mn);
     if(mn==1)c1++;
    if(mn==2)c2++;
...
// This function enables connecting elements along each column, very similar to what has been done for connecting along the rows.
...
void multiply(node * row1, node * col2){
    node * colH=col2;
    node * rowH=row1,*rowtemp,*a,*b,*rowHtemp,*c,*r;int sum,flag=0;
    node * tempcol;node * temprow;
    while(rowH){
           colH=col2;
        while(colH){
            sum=0;
...
int main(){
    ...
    col2=linkMat(n,row2);  
    if(c1==0 || c2==0){//Condition that results in the boundary case where both or one of the matrices is
        printf("NULL MATRIX!");
    }
    else{
        multiply(row1,col2);
